...
\begin{tabular}{llcccccc}
  ...
  Craftax & small & 8 & 32.447 & 32.247 & 0.200 & 32.247 & 0.200 \\
  Craftax & medium & 8 & 34.031 & 34.031 & 0.000 & 34.031 & 0.000 \\
...
\begin{tabular}{@{}lllcc@{}}
  ...
  Craftax & small & \(L=1\), \(E=1\), \(d=128\) & \(1.00\) & \(32.447\) \\
  Craftax & medium & \(L=1\), \(E=1\), \(d=256\) & \(3.00\) & \(34.031\) \\
...
\begin{tabular}{@{}llllrrr@{}}
  \toprule
  Domain & Budget & Best \(L=4\) & Best \(L=2\) & \(\Delta Q\) & \(n\) & \(p\) \\
  \midrule
...
\begin{tabular}{@{}llllrrr@{}}
  \toprule
  Domain & Budget & Best \(L=2\) & Best \(L=1\) & \(\Delta Q\) & \(n\) & \(p\) \\
  \midrule
...
For each small and medium budget, the \(L=4\) configuration with the best observed mean return is compared with every \(L=2\) configuration. Positive \(\Delta Q\) favors \(L=4\), and \(p\) is from the one-sided Welch test defined above. \(n\) gives the number of independent runs per configuration.
}
\begin{center}
\small
...
\begin{tabular}{@{}lllrrr@{}}
  \toprule
  Budget & Best \(L=4\) & \(L=2\) configuration & \(\Delta Q\) & \(n\) & \(p\) \\
  \midrule
...
For each small and medium budget, the \(L=2\) configuration with the best observed mean return is compared with every \(L=1\) configuration. Positive \(\Delta Q\) favors \(L=2\), and \(p\) is from the one-sided Welch test defined above. \(n\) gives the number of independent runs per configuration.
}
\begin{center}
\small
\setlength{\tabcolsep}{5.0pt}
\begin{tabular}{@{}lllrrr@{}}
  \toprule
  Budget & Best \(L=2\) & \(L=1\) configuration & \(\Delta Q\) & \(n\) & \(p\) \\
  \midrule
...
The Sokoban small- and medium-budget rows contain nine runs. Seeds 7 and 9 for
the small-budget \(L=1,E=1,d=128\) anchor finished at \(92.9\%\) and \(92.2\%\)
of the nominal training steps, respectively; both are included using the same
final-five-return summary as the other runs. Craftax small and medium use nine
runs per configuration; large-budget Sokoban and Craftax use the available
three runs.

\setcounter{table}{6}
\input{tables/fixed_compute_seed_tables}
\clearpage
