...
def _resolve_scratch_root() -> Path:
    env_root = os.environ.get("NANOGPT_SCRATCH_ROOT")
    if env_root:
...
DEFAULT_CKPT_DIR = os.environ.get(
    ...
    str(
        _resolve_existing_path(
            DEFAULT_SCRATCH_ROOT / "nanogpt" / "ckpts",
            DEFAULT_SCRATCH_ROOT / "nanogptjax_checkpoints",
        )
    ),
)
...
class MultiHeadAttentionConfig:
    ...
    def __post_init__(self):
        init = init_uniform(scale=3**0.5 * self.d_in**-0.5)
...
class ModelConfig:
    ...
    kv_heads: int = 4
    attn_type: str = "gqa"
    experts_aggregation_regime: str = "sum_div_sqrt_num_experts"
    ...
    def __post_init__(self):
        if self.model_backend not in ("gpt", "moe", "shared_kv"):
            raise ValueError(
                "`model_backend` must be one of `{'gpt', 'moe', 'shared_kv'}`."
            )
        ...
        if self.experts_aggregation_regime not in (
            "mean",
        ...
        if self.d_emb % self.q_heads != 0:
            raise ValueError(
                f"`d_emb` must be divisible by `q_heads`, got {self.d_emb} and {self.q_heads}."
            )
        if self.q_heads % self.kv_heads != 0:
            raise ValueError(
                f"`q_heads` must be divisible by `kv_heads`, got {self.q_heads} and {self.kv_heads}."
            )
...
class HyperParams:
    ...
    desired_batch_size: int = 131072 #524288 #262144 #131072 #524288 #262144 #131072 #262144 #4048 #524288 #128
