...
class GroupedQueryAttention(ParamInitializer):
    ...
    def param_specs(cls, cfg):
        wq = ParamSpec(
            shape=(cfg.d_in, cfg.q_heads, cfg.head_dim),
            dtype=cfg.dtype,
            logical_axes=cfg.wq_logical_axes,
            initializer=cfg.wq_initializer or linear_init(cfg.d_in, cfg.head_dim),
        )
        wk = ParamSpec(
            shape=(cfg.d_in, cfg.kv_heads, cfg.head_dim),
            dtype=cfg.dtype,
            logical_axes=cfg.wk_logical_axes,
            initializer=cfg.wk_initializer or linear_init(cfg.d_in, cfg.head_dim),
        )
        wv = ParamSpec(
            shape=(cfg.d_in, cfg.kv_heads, cfg.head_dim),
            dtype=cfg.dtype,
            logical_axes=cfg.wv_logical_axes,
            initializer=cfg.wv_initializer or linear_init(cfg.d_in, cfg.head_dim),
        )
        wg = None
        if cfg.use_output_gate:
            wg = ParamSpec(
                shape=(cfg.d_in, cfg.q_heads, cfg.head_dim),
                dtype=cfg.dtype,
                logical_axes=cfg.wg_logical_axes,
                initializer=cfg.wg_initializer or jax.nn.initializers.zeros,
            )
        wo = ParamSpec(
